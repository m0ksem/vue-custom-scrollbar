import { firstLetterToCapital } from "./words"
...
export const percent = (value: number | string, variableName = 'value') => {
  ...
  throw new Error(`${firstLetterToCapital(variableName)} must be string number in percents or just a number. For example 40%.`)
}
